...
Sistem dilengkapi dengan sekumpulan skrip pengujian untuk memvalidasi performa deteksi, ketahanan konfigurasi, dan kelayakan komputasi waktu nyata (real-time feasibility):

| Skrip | Fungsi & Metrik Evaluasi |
|-------|--------------------------|
| `run_full_test.py` | Mengukur **Akurasi Deteksi** (Precision, Recall, F1-Score) pada 5 skenario serangan (total 500 baris) berdasarkan data hasil randomisasi. |
| `run_latency_test.py` | Mengukur **Kelayakan Waktu Nyata** (Latency & Throughput) dengan menduplikasi data hingga 10.000 baris simulasi. |
...
| `run_batch_test.py` | Menguji keandalan sistem (*Stress-test*) dalam memproses `testing_dataset.csv` berskala besar (125.001 baris). |
| `generate_testing_dataset.py`| Generator dataset sintetik terinjeksi anomali secara dinamis (menggunakan randomisasi) untuk 5 skenario dasar dan file *stress-test*. |
...
Dataset sintetik dibangkitkan menggunakan script `generate_testing_dataset.py` untuk mensimulasikan pergerakan kapal yang normal maupun terinjeksi anomali GPS Spoofing secara dinamis.
...
Evaluasi metrik klasifikasi menggunakan 5 skenario dasar berukuran masing-masing 100 baris. Nilai anomali disuntikkan secara dinamis pada rentang titik tertentu:

| File | Skenario | Keterangan Anomali |
|------|----------|--------------------|
| `normal_route.csv` | Rute Normal | Baseline pergerakan wajar di dalam wilayah operasi. |
| `sudden_jump.csv` | Lompatan Koordinat | Perpindahan posisi mendadak yang memicu *Speed Alert*. |
| `slow_drift.csv` | Pergeseran Arah | Pergeseran *Course over Ground* (COG) bertahap. |
| `geofence_escape.csv`| Keluar Zona Natuna | Koordinat dimanipulasi ke luar batas wilayah Geofence. |
| `mixed_attack.csv` | Serangan Campuran | Kombinasi acak dari berbagai anomali serangan. |
...
|----------------|-------|------------------|
| Duplikasi Memory | 10.000 | Mengukur *latency* & *throughput* pada `run_latency_test.py`. |
| `testing_dataset.csv` | 125.001 | *Stress-test* panjang pada `run_batch_test.py` untuk menguji memori dan stabilitas sistem secara masif. |
| `full_test_summary.csv` | 6 | Ringkasan metrik akurasi (Precision, Recall, F1) hasil ekstraksi `run_full_test.py`. |
